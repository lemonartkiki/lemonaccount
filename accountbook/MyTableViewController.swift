...
class MyTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
        ...
        //        cell.lblAmount.text = currDic["shoppingamount"] as? String
        print("=========\(cell.lblAmount)")
        //回傳準備好的儲存格
        return cell
    }
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath)
    {
        ...
        let test = self.arrTable[indexPath.row]["time"]! as! String
        print(test)
    }
    ...
    {
        //        let moreAction = UITableViewRowAction(style: .normal, title: "到最前面") { (rowAction, indexPath) in
        //            print("置頂按鈕被按下！")
        ...
        let deleteAction = UITableViewRowAction(style: .destructive, title: "刪除") { (rowAction, indexPath) in
            ...
            let sql = "delete from account where time = '\(self.arrTable[indexPath.row]["time"]! as! String )'"
            print("~~~~刪除指令 \(sql)")
            //將ＳＱＬ指令轉成Ｃ語言字串
            let cSQL = sql.cString(using: .utf8)
            //宣告儲存指令結果的指令
            ...
            /*
             ...
             最後一個參數為預留參數，目前沒有作用）
             */
            sqlite3_prepare(self.db, cSQL, -1, &statement, nil)
            //Step1-3:準備執行ＳＱＬ指令
